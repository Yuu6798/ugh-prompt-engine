...
指摘）。これを防ぐため、`compose_payload` は**両方の publish**（1 本目の
`payload.md` の呼び出しから 2 本目の `payload.manifest.json` の完了まで）
を単一の try/except で包み、except ハンドラへ例外が届いたら**両方の出力**
（`missing_ok=True` で「実際には publish されていない側」も安全に扱う）を
unlink してから元の例外を再送出する——publish は「両方揃う」か「どちらも
残らない」かの 2 状態に限定する（rollback）。どちらかの呼び出し内部の書き
込みそのものが失敗した場合だけでなく、内部の `os.replace` が完了して該当
ファイルが publish 済みになった直後・呼び出しが return する前に非同期例外
（`KeyboardInterrupt` 等）が届いた場合（1 本目・2 本目のどちらの publish
直後でも）も同じハンドラでカバーする（片方の出力のみの unlink で扱うと、
もう一方の経路で逆向きの部分残留——例:「manifest あり payload なし」——が
起きる——2 本を独立に unlink し、片方が失敗してももう片方は試行する）。
unlink 自体が失敗した場合は隠さず、元の例外へ失敗した unlink の文脈を付け
て送出する。トレードオフとして、**両方の publish が成功した後・try ブロッ
クを脱出する前**に非同期例外が届いた場合も同じハンドラが走り、成功済みの
バンドルが unlink される——「両方揃う」か「どちらも残らない」かの 2 状態
契約には整合する（再実行すれば同一バイト列で再度両方揃う）ため許容する。

**境界宣言**: この rollback がカバーするのは「1 本目の publish 開始から
2 本目の publish 完了後 try ブロックを脱出するまでの間に、例外が上記
except ハンドラへ届く全ケース」。残る窓は **Python の例外機構を経由しない
プロセス即死のみ**（シグナルによる強制終了等）——構造的な 2 ファイル原子性
（ディレクトリ単位の swap 等）は本計器のスコープ外の再設計を要する。運用
上、そのウィンドウでの残留を検出した場合（`payload.md`/`payload.manifest.
json` のどちらか一方のみ存在）は「不完全公開」として手動で残留ファイルを
...
def compose_payload(manifest_path: Path, out_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        # Rollback publish: either `atomic_write_bytes` call inside this try
        # block can hand an exception to this handler — the 1 本目
        # （`payload_path`）の書き込み自体が失敗した場合、2 本目
        # （`manifest_out_path`）の書き込み自体が失敗した場合、さらに内部の
        # `os.replace` が完了して該当ファイルが publish 済みになった直後・
        # 呼び出しが return する前に非同期例外（`KeyboardInterrupt` 等）が
        # 届いた場合（1 本目・2 本目のどちらの publish 直後でも）を含む。
        # 両出力を unlink して「両方揃う」か「どちらも残らない」かの 2 状態
        # へ戻す。`payload_path` のみ／`manifest_out_path` のみを想定した
        # 片側 unlink だと、もう一方の経路で逆向きの部分残留（例:
        # 「manifest あり payload なし」）が起き、再実行は残った側の上書き
        # 禁止チェック（`ProtectedPathError`、本関数冒頭）で拒否される。
        # `missing_ok=True` はどの経路でも（unlink 対象が実際には publish
        # されていない側でも）安全に動作させるため。2 unlink は独立に試み、
        # どちらかが失敗しても残りは試行する（1 つ目の失敗で 2 つ目を諦める
        # と、除去できたはずのファイルまで残留させてしまう）。
        ...
        if rollback_errors:
            raise OSError(
                f"payload publish failed ({exc!r}) and rollback unlink failed "
