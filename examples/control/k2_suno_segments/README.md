...
4 run の生成時刻 t1..t4（ABBA 順）に対し **B = |(t2+t3) − (t1+t4)| / (t4−t1)** を
受領時に計算し、**B ≤ 0.1 で均衡（canonical 資格）/ B > 0.1 で confounded を
pre-mark**。B は low run 平均時刻と high run 平均時刻のずれをバッチ全長で正規化
した量（完全等間隔 ABBA なら B=0）で、序数的 ABBA でも実時刻非対称（キュー遅延
等）ならドリフト交絡が残るため、タイムスタンプは記録だけでなくゲートにする。
閾値 0.1 は全セッション幅ドリフトの 1/10 未満に漏れを抑える保守慣行値（K 系列の
0.2/0.8 と同じ規約閾値として事前登録）。実務含意: 4 run は間隔を空けず連続実行。
