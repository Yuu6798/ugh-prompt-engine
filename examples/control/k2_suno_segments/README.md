...
**primary verdict: dead（canonical）** — high セルはヌル（low セル）に対する優位
ゼロ。**判定履歴（訂正の透明性）**: #166 では納品順（high 全件 → low 全件）から
「交互生成順は未実施＝順序が cell と完全共線」と推定し、primary verdict を一度
「測定済みだが confounded・未確定」へ格下げした（Codex P2 第 4 ラウンド採用）。
その後 2026-07-10 の生成者本人への追確認で、実際の生成順は **run 単位の厳密交互**
（low run→high run→low run→high run・各 run 2 テイク・補充 2 本は末尾）と証言が
得られ、当初推定は誤りと確定 — セル×生成時刻の完全共線は存在せず時間ドリフトは
両セルへ対称分散していたため、格下げの根拠が消滅し、事前登録規則の機械適用結果
（dead）を canonical へ復元した（`demotion_reversal_note`）。証言は生成後の
追確認・独立証跡（UI タイムスタンプ）未取得の attestation-tier（model 申告と同格、
honesty (3)）。high セル単独への規約適用は 0.666667 → loose だが、low セル
（ヌル）が同値 = chance floor のため単独読みを grip の証拠にはしない。処方 outro
静音化の実現 **0/4**・完全一致ゼロ（下記「解釈」）は処方非実現の記述的証拠として
併記（`descriptive_evidence`）。
...
**#164 型の cross-batch 再利用交絡を回避**し、さらに生成順も run 単位の厳密交互
（証言ベース、honesty (3)）で時間ドリフトが両セルへ対称分散していたため、
隔離設計は cross-batch・バッチ内順序の両面で成立し **dead 判定を canonical に
確定**できた。**判定履歴**: #166 レビュー時点では納品順からの推定で「順序が cell
と完全共線」とみなし一度 confounded へ格下げしたが、生成者証言により推定が
誤りと確定し復元した — 納品順は生成順の証跡にならない（provenance は生成時に
記録すべき）という教訓ごと透明に記録する。**次バッチ要件: 生成順の同時記録**
（証言の attestation-tier を UI タイムスタンプ等の独立証跡へ引き上げる）。
