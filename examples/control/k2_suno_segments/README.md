...
事前登録済み（#168 P2 採用）: primary は**スロット保存補充**（各 run 直後に曲長
≥30s を確認し、短尺は次の run へ進む前に同セルを即時再生成して同スロットを埋める
— ABBA 位置バランス保存・再生成もタイムスタンプ記録）。スロット保存が守れず
末尾補充になった場合、そのバッチは**ヌル格下げ比較について order-confounded を
自動 pre-mark**（high セル単独規約と記述的証拠は有効のまま。paired 末尾補充は
位置平均が回復しないため不採用）。
