...
事前登録済み（#168 P2 第 2〜3 ラウンド採用で単純化）: スロット保存補充（各 run
...
high={4,5,6,7}）。よって**いかなる補充（スロット保存・末尾を問わず）が発生した
バッチも、ヌル格下げ比較は order-confounded を自動 pre-mark** — その場合の
verdict は high セル単独規約 vs 解析的 chance floor（直交処方でデフォルト形状
との一致が 1/3 と既知）に立脚する。**補充ゼロで完走したバッチのみヌル格下げ
比較が canonical 資格**（paired/filler 規則は既存受入テイクを捨てない限り
位置平均が回復しないため不採用）。
