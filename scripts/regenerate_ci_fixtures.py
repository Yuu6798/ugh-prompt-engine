...
def build_run(target_path: Path, observed_path: Path) -> SemanticCIRun:
    ...
    return run_semantic_ci(target, observed)
...
def render_result(target_path: Path, observed_path: Path) -> str:
    ...
    result = build_run(target_path, observed_path)
    return json.dumps(result.model_dump(mode="json"), ensure_ascii=False, indent=2) + "\n"


def render_report(target_path: Path, observed_path: Path) -> str:
    """Render semantic CI output using the same Markdown shape as `svprpe ci-check`."""

    return render_markdown(build_run(target_path, observed_path))
...
def render_report_scenario(name: str) -> tuple[Path, str]:
    scenario_dir = FIXTURE_DIR / name
    expected_path = scenario_dir / "expected_report.md"
    target_path = scenario_dir / "target_svp.json"
    observed_path = scenario_dir / "observed_rpe.json"
    if not target_path.is_file():
        raise FileNotFoundError(f"missing fixture: {target_path}")
    if not observed_path.is_file():
        raise FileNotFoundError(f"missing fixture: {observed_path}")
    return expected_path, render_report(target_path, observed_path)
...
def regenerate() -> int:
    for scenario in SCENARIOS:
        ...
        report_path, report_text = render_report_scenario(scenario)
        report_path.write_text(report_text, encoding="utf-8")
        print(f"[regenerate] wrote {report_path.relative_to(ROOT).as_posix()}")
    return 0
...
def check() -> int:
    ...
    for scenario in SCENARIOS:
        ...
        if not report_path.is_file():
            mismatches.append(f"{scenario}: expected_report.md missing")
            continue
        expected_report = report_path.read_text(encoding="utf-8")
        if expected_report != report_text:
            mismatches.append(f"{scenario}: expected_report.md is out of date")
