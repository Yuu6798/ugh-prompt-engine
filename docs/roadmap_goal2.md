...
> スコアでの一般化が残り。
>
> **n=20 スケールアップ（2026-07-03）**: 同一スコア・同一プロンプトで
> seed 5000–5019 を実測（`examples/roundtrip/musicgen_r3_rep_n20_2026-07-03.json`）。
> key 保存率は **3/20=0.15 に確定**（n=5 の 0.4/0.0 の揺れは二項ノイズ内で
> 真の保存率は低いと判明）、rejection sampling は 20 中 2 本の両フィールド保存
> テイクで**完全回復**した。詳細は
> [`musicgen_backend.md`](musicgen_backend.md) §7.3。**依然として単一スコア・
> 単一生成器の結果** — Suno バッチ / 複数スコアでの一般化は未実施。
