...
| ID | 成果物 | 受け入れ条件 |
|---|---|---|
| R1-1 | 往復 manifest スキーマ — `{プロンプト全文, 生成器, 真値(意図 bpm/key/拍子等), 計測値, 音源ハッシュ, 音源ロケータ(コミット相対パス or アーティファクト URI)}` を 1 ケース = 1 レコードで保存 | 既存 4 ケース（roundtrip_case_studies §1）が manifest 形式で再記述できる。**校正可能 corpus では音源ロケータが必須**で、そこからハッシュ一致音源を解決できる（プロンプトのみの観測ログはロケータ空欄可） |
...
**保存音源（CC0 コミット or ハッシュ一致アーティファクト）** から往復を
再実行できる。プロンプトのみのケースは校正可能 corpus に数えない。
