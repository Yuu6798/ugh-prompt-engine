...
予算**として計上する。分離込み（S-fullstack）でも 0.33% と低いが、これは
...
（S-direct 1.000 / V-direct 0.998・有声の取りこぼしなし）に対し、VFA は S-direct
...
将来作業）。M3 既定の根拠は「原因が何であれ抽出器の voicing 判定は信頼できない」
という実測事実であり、リリース尾仮説の真偽には依存しない。**M3 への含意（最重要）**:
比較器は抽出器の voiced/unvoiced 判定を信用してはならない。**両系列が共に有声の
フレームのみで音高比較**し、voicing 不一致をペナルティ化しない設計を既定とする
（est_voiced_confidence_floor 0.30 は凍結値・事後調整禁止）。ただし**被覆の正直会計を
併設する**: 共有有声フレーム限定は「歌唱区間の削除・オンセット移動・部分重複」を
評価から消しうるため、M3 は joint-voiced 被覆率（有声区間の IoU 相当）を**独立信号
として必ず併記し、被覆下限ゲートを下回る比較を類似度として報告しない**こと
（僅かな重複残存での過大類似を防ぐ）。
...
- **§7 分岐の判定は User 決裁待ち（本 doc は判定を先取りしない）**。記録された verdict
  ...
  緊張関係にある。選択肢は (a) 文言どおり M3 非前進（S-direct の正解境界修正 or 代替
  抽出器での dated 再入を待つ）/ (b) §7 を「音高系と voicing 系の撤回範囲を分離する」
