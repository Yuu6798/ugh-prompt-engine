...
- **主センサー**: match_rate_low_cell=0.666667 / match_rate_high_cell=0.666667。
  ...
  （0.666667 ≤ 0.666667）で初発動**し、**primary verdict: dead（canonical）** —
  high セルはヌルに対する優位ゼロ。判定履歴（訂正の透明性）: #166 では納品順
  （high 全件 → low 全件）から順序共線を推定し一度「測定済みだが confounded・
  未確定」へ格下げしたが、生成者証言（2026-07-10 追確認）で実際の生成順は
  run 単位の厳密交互（時間ドリフトは両セルへ対称分散）と確定し、格下げの根拠が
  消滅したため事前登録規則の機械適用結果（dead）を canonical へ復元した。
  order provenance は証言ベース（独立証跡未取得の attestation-tier、model 申告と
  同格）。high セル単独の規約適用は loose だが、low セル（ヌル）同値 =
...
- **同一バッチ隔離設計の初適用**: 経済化のためバッチ 1 `calm` を low
  ...
  生成順も run 単位の厳密交互（証言ベース）で順序ドリフトは両セルへ対称分散 —
  ただし #166 レビュー時点では納品順からの推定で順序共線とみなし一度 confounded
  へ格下げし、生成者証言による訂正で dead canonical へ復元した経緯を透明に記録
  （納品順は生成順の証跡にならない。次バッチ要件 = 生成順の同時記録で
  attestation-tier を独立証跡へ引き上げる）。
- **計器知見**: 3 区間・処方 `[low, high, low]` は match_rate が
  `{0, 0.333, 0.667, 1.0}` の 4 値しか取れず分解能が粗い（本バッチ canonical
  計測は 8/8 が 0.667）。バッチ 3 は loud–quiet–loud 等、生成器デフォルト形状と
  正面から対立する直交処方（chance floor 1/3）による頑健化 — 位置づけは
  「dead 確定のための追試」ではなく**別の問い**（デフォルト形状を処方で上書き
  できるか）の測定。
