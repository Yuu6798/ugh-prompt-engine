...
- **計器知見**: 3 区間・処方 `[low, high, low]` は match_rate が
  ...
  **ABBA カウンターバランス（low→high→high→low の run 順 — 線形ドリフトの位置
  ...
  補充経路も事前登録する（#168 P2 採用）: **primary はスロット保存補充** — 各 run
  直後に 2 テイクの曲長 ≥30s を確認し、短尺があれば次の run へ進む前に同セルを
  即時再生成して同スロットを埋める（ABBA 位置バランス保存・再生成もタイムスタンプ
  記録）。**スロット保存が守れず末尾補充になった場合、そのバッチはヌル格下げ比較に
  ついて order-confounded を自動 pre-mark** する（high セル単独規約と記述的証拠は
  有効のまま）。（paired 末尾補充は不採用 — 既存受入テイクを捨てない限り対 run の
  テイクに使用枠がなく、位置平均は回復しない。）
