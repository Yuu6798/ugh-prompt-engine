...
- **計器知見**: 3 区間・処方 `[low, high, low]` は match_rate が
  ...
  **B = |(t2+t3) − (t1+t4)| / (t4−t1)** を受領時に計算し、**B ≤ 0.1 で均衡
  （canonical 資格）/ B > 0.1 で confounded を pre-mark**。B は low run 平均時刻と
  high run 平均時刻のずれをバッチ全長で正規化した量（完全等間隔 ABBA なら B=0）で、
  序数的 ABBA でも実時刻非対称（ある run 前のキュー遅延等）ならドリフト交絡が
  残るため、タイムスタンプは記録するだけでなくゲートにする。閾値 0.1 は「全
  セッション幅ドリフトの 1/10 未満に漏れを抑える」保守慣行値（K 系列の 0.2/0.8 と
  同じ規約閾値として事前登録）。実務含意: 4 run は間隔を空けず連続実行する。
