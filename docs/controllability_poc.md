...
- **計器知見**: 3 区間・処方 `[low, high, low]` は match_rate が
  ...
  **ABBA カウンターバランス（low→high→high→low の run 順 — 線形ドリフトの位置
  ...
  補充経路も事前登録する（#168 P2 第 2〜3 ラウンド採用で単純化）: スロット保存
  ...
  位置は low={1,2,8,9} vs high={4,5,6,7}）。よって**いかなる補充（スロット保存・
  末尾を問わず）が発生したバッチも、ヌル格下げ比較（cross-cell）は
  order-confounded を自動 pre-mark** し、その場合の verdict は **high セル単独
  規約 vs 解析的 chance floor** に立脚する — バッチ 3 は処方直交化により
  デフォルト形状との一致が解析的に 1/3 と既知のため、経験的ヌルが confounded
  でも high セル単独の読みが成立する（直交処方の設計価値）。**補充ゼロで完走した
  バッチのみヌル格下げ比較が canonical 資格**。（paired/filler 規則は不採用 —
  既存受入テイクを捨てない限り位置平均は回復しない。）
