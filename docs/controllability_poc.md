...
- **計器知見**: 3 区間・処方 `[low, high, low]` は match_rate が
  ...
  **ABBA カウンターバランス（low→high→high→low の run 順 — 線形ドリフトの位置
  ...
  補充経路も事前登録する（#168 P2 第 2〜4 ラウンド採用で最終化）: スロット保存
  ...
  位置は low={1,2,8,9} vs high={4,5,6,7}）。最終規則 3 点: (1) **いかなる補充
  ...
  補充ゼロで完走した ABBA バッチ**。(3) 補充発生時、位置均衡は事後修復不能の
  ため canonical 判定には**新規バッチの再走**を要する（restart 規則の事前登録。
  paired rerun / filler は不採用維持 — 既存受入テイクを捨てない限り位置平均は
  回復しない）。
