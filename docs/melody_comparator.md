...
**既知の限界（レビュー判定 2026-07-30・phrase 境界の絶対秒依存）**: phrase
分割境界（`phrase_gap_sec` 0.6s・M1 凍結値継承）は絶対秒であり、テンポ変形が
境界を跨ぐとフレーズ構造が変化しうる。比較器の不変性主張は音程列・IOI 比列に
対するものでフレーズ構造には及ばない。この境界感度は M3d の tuning 変形分布
（±8〜15%）に内包され、校正された閾値の目盛りに折り込まれる（凍結値変更は
実測なしに行わない）。

**既知の限界（レビュー判定 2026-07-30・IOI比/音長比 log2 量子化の境界問題・
第 21 ラウンド不採用記録）**: IOI 比・ノート長比の log2 量子化（0.25 刻み・
事前登録凍結値）は、比がビン境界近傍に乗る場合、浮動小数点ノイズでテンポ変形
の前後にビンが反転しうる。比は厳密算術ではテンポ不変であり、これは量子化に
内在する測度ゼロ境界の問題で、tie ルールでは原理的に除去できない。実抽出ペア
では onset ジッタが支配的であり、M3d の tuning 変形分布に折り込まれる
（2026-07-30 レビュー判定・凍結刻みの意味論変更は実測なしに行わない）。
...
4. 軸類似（整列カラム上のみ）:
   ...
   - `rhythm` = duration 比較（全整列カラム）+ IOI 比較（直前カラムも連続整列の
     カラムのみ）をプールした一致率
   - **全軸算出不能ガード**（レビュー対応 2026-07-30 第 21 ラウンド）: 被覆下限
     ゲートは通過したが整列 interval カラムが 0（全フレーズが単ノートで
     `intervals_folded` が全フレーズで空になる縮退）だと 3 軸すべて `None` に
     なる。この場合は `evidence="none"` へ進まず、`not_comparable`（理由
     `no_computable_axes`。coverage は観測事実として出す）を返す。
