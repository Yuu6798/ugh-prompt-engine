...
| ID | 成果物 | 受け入れ条件 | Status |
|---|---|---|---|
| P3-01 | `src/svp_rpe/batch/report.py` 実装 | CSV + Markdown 出力、ランキング/統計込み | ⏳実装済み — 実体は `src/svp_rpe/batch/runner.py` の `render_summary_csv`/`write_batch_outputs`、`--output-dir` 経由で CSV/MD 出力。`batch/report.py` は存在しない（`batch/` の実体は `discovery.py`（ファイル探索）と `runner.py`（実行 + レポート生成）の 2 ファイル構成） |
