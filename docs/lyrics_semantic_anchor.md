...
## n=3 追試（2026-07-01 Session 2）：dynamic_range 説の棄却 + mid_ratio の確証
...
### 結論 2：`mid_ratio` はジャンルも再生成も跨いでボーカルを拾う

**歌詞ありテイク 4 本すべて**が、それぞれの instrumental より高い mid_ratio を示した
（EDM: 0.226/0.221 > 0.217、Rock: 0.245/0.229 > 0.208）。2 ジャンル × 再生成ノイズを
跨いで向きが一貫する。前節の「mid_ratio はボーカル検出に堅い(2/2)」を強化し、
**現状ボーカル存在を安定検出できる唯一の物理センサーは mid_ratio**、と位置づける。
...
- **`dynamic_range`→歌詞 の棄却（n=3 で確定）**：EDM 限定で成立するが Rock で反転し、
  ...
  物理代理指標にはできない。ボーカルの安定した物理検出子は **mid_ratio のみ**に絞る。
