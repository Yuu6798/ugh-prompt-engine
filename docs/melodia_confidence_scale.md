...
- demucs stem の噛み合わせ問題は、**stem 対照を実測した `kane_y2` については棄却**
  （§2.4。実経路の stem 上で Melodia 自身が 48.1% のフレームに音高を出し、同一 stem で
  pyin は床を越える）。他 4 素材は stem 対照未実施（生 mp3 は `crslv2_w3` まで測定済み）
...
| demucs stem との噛み合わせ問題 | **測定した `kane_y2` については否**。§2.4 で実経路を全長素材に回し、Melodia 自身が 48.1% のフレームに音高を出すこと、**同一 stem** で pyin は 1100 フレームが床を越えることを確認した（この stem の欠陥では説明できない）。他 4 素材の stem は同等の対照未実施のため、素材固有の stem アーティファクトの可能性はそれらについて未棄却 |
