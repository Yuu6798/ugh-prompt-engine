...
> プロンプトへ出す物理欄は **brightness / bpm / key と構造の density 語のみ**。
> `time_signature` は renderer に segment が無く **未送出**。`stereo_width` /
> `active_rate_target` / `valley_depth_target` は最低優先の `physical.optional` 行に
> あり、J-ebm プロンプトは長く `prompt_max_chars=650` で truncate されたため
> **これも未送出**。よって本 A/B で実際に固定・送出された物理ノブは
> **bpm / key / brightness の 3 つだけ**。残りの欄の一致は「制御の証拠」ではなく
> 既定/スタイルによる結果として読む。

| 計測欄 | Suno に送出? | J-rock | J-ebm | 読み |
|---|---|---|---|---|
| bpm | ✅ 送出 (175) | 89 | 89 | **判定不能** — 両値とも §4 の 89.1 アトラクタの可能性。Suno が 175 を生成した証拠が無く「再現」とは言えない |
| key | ✅ 送出 (E major) | E major | E major | 一致＝送出ノブが再現 |
| brightness | ✅ 送出 (bright) | 3696 (bright) | 3284 (bright) | 同帯域＝送出ノブが再現 |
| 拍子 | ❌ 未送出 | 4/4 | 4/4 | **制御の証拠でない**（既定/スタイル） |
| active | ❌ 未送出 | 0.992 | 0.998 | 制御の証拠でない（結果） |
| valley | ❌ 未送出 | 0.034 | 0.045 | 制御の証拠でない（結果） |
| stereo | ❌ 未送出 | 0.398 | 0.243 | 制御の証拠でない・かつ未校正 |
...
1. **送出した物理ノブのうち key / brightness は再現した** — 意味層を全面差替しても
   ...
   89.1 アトラクタと交絡し、175 が保たれた証拠にならない。さらに
   **time_signature / active / valley / stereo はプロンプトに渡っておらず、一致しても
   制御の証拠にならない**（既定/スタイル）。meter/density を本当に制御変数にするには
   renderer への追加か prompt 予算の調整が、bpm の判定には音源/真値の保存が要る。
...
未送出欄（meter/density/stereo）は本 A/B の対象外。
...
| **T2** | 送出かつ計器が信頼できるノブ（**key / brightness のみ**）で往復一致を確認。bpm は判定不能、meter/active/valley/stereo は未送出で検証対象外（§3）。本ケースは実生成器版の T2 先取りだが、検証済み欄を上記に限定して引き継ぐこと |
