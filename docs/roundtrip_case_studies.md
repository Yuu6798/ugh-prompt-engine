...
|---|---|---|---|---|
| bpm | ✅ 送出 (175) | 89 | 89 | 一致＝送出ノブが再現（※BPM 計器の留保は §4） |
...
1. **送出した物理ノブ（bpm / key / brightness）は再現した** — 意味層を全面差替しても
   この 3 欄は一致。= プロンプトに渡した物理指示を演奏者が保った。**双方向再現性は
   送出した物理欄で成立**。ただし **time_signature / active / valley / stereo は
   プロンプトに渡っておらず、一致しても制御の証拠にならない**（既定/スタイル）。
   meter/density を本当に制御変数にするには renderer への追加か prompt 予算の調整が要る。
...
**総合判定**: 双方向性テストは、**実際に送出した物理ノブ（bpm/key/brightness）について
成功**（BPM 計器の留保付き・ユーザー合意）。未送出欄は本 A/B の対象外。
