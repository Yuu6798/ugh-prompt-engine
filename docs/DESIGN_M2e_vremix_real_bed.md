...
- **`S`**: プロセスプールの起動〜モデルロード完了までの固定コスト（秒）。シャードごとに
  1回だけ発生する。
  **測定方法の明確化（rev.8・2026-08-05・User 決裁「案 A」）:** 実装は遅延ロード
  （モデルは各ワーカーの初回セル内で初期化される）のため、「プール起動〜モデルロード
  完了」はプール起動ギャップと各アームの初回セル内ロードに分散して現れる。よって
  `S` は次の 3 項の和として run store の実測から導出する:
  1. **プール起動ギャップ**: run 開始（run record の `started_utc`）から最初のセル
     開始（store 内最小の `measurement_started_utc`）までの秒数
  2. **初回 direct セルのロード超過**: 当該 run の direct アーム最初のセルの
     `elapsed_seconds` − 同 run の direct アーム 2 セル目以降の中央値
  3. **初回 stem セルのロード超過**: stem アームについて同様
  この導出は保守側（`S` を大きめに見積もり cap を小さくする側）に働く。
  **セッション毎の `S` 再導出手順:** 各セッション開始時の 2 波再校正（本節既定）の
  store に対して上記 1–3 を同一手順で適用し、そのセッションの地図引き直しに使う。
  前セッションの `S` の持ち越しは `T_*` と同様に禁止する。
  P=2 校正 run（2026-08-05, store_calib_p2）での実測: ギャップ 77.0 s +
  初 direct 超過 103.9 s + 初 stem 超過 156.5 s = **S = 337.4 s**。
...
- **`R_max = 21 回`**（`N_shards` の上限）を宣言する。
  （rev.8・2026-08-05: 18 から §8.8 所定の User 決裁（条件付き決裁形式）を経て改訂。
  P=2 再校正（r2-0 再実行）で S=337.4 s / T_direct=28.48 s / T_stem=138.59 s が確定し、
  地図生成器が N_shards=19 > 18 で fail-closed 停止したため。User は事前に
  「N_shards ≤ 22 ならば R_max = N_shards + 2 として確定し進行してよい」と
  数字を添えて決裁済み（§8.8 の 3 択のうち「1. R_max を引き上げる」を条件付きで採用）。
  N_shards=19 ≤ 22 につき R_max = 19 + 2 = 21。
  決裁記録: `docs/measurements/m2e_2026-08/r_max_decision2_2026-08-05.md`。）
  （rev.7・2026-08-05: 当初の凍結値 12 から §8.8 所定の User 決裁を経て 18 へ改訂。
  r4 実測で実行可能なあらゆる `P` において `N_shards > 12` が確定したため。
  決裁記録: `docs/measurements/m2e_2026-08/r_max_decision_2026-08-05.md`。
  3 択のうち「1. R_max を引き上げる」を採用——規模は不変・回数だけ増える。）
