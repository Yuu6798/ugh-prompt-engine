...
- **（Codex レビュー第 12 ラウンド追補・T2/F1 の役割再定義）** T2/F1（上記）
  は「pin 照合〜`librosa.load` 読込の窓」の防御として書かれていたが、
  「decode 中だけ WAV を一時置換し、T2/F1 の**前**に pin 一致の原本へ戻す」
  updater 相当の攻撃には無力だった——T2/F1 は decode 完了**後**の原本を
  再照合するだけで、decode が実際に消費したバイトそのものへは一切束縛して
  いなかったため、未承認バイト由来の変形が「承認済み provenance」を掲げて
  公開されうる穴が残っていた。`generate_vocadito_variants` に**スナップ
  ショット束縛**（staging 配下へ decode 直前にコピーし、コピー先のバイトを
  非キャッシュで pin 照合してから、その同じスナップショットパスを
  `librosa.load` へ渡す。`harness._freeze_audio_copy` と同型）を導入し、
  「decode が消費したバイト = pin 照合したバイト」を構造的に保証した——
  ここで**役割を再定義する**: スナップショット束縛 = **消費バイト束縛の正**
  （decode 経路が pin 照合と異なるバイト列を読む余地が構造的に存在しない）、
  T2/F1（公開直前再照合）= **ビルド環境静穏性の検査**（ビルド実行中に原本が
  外部要因で変化していないかを見る計器。decode バイト自体の束縛にはもはや
  関与しない・多層防御として残置）。この役割分担を各実装のコメント/
  docstring に明記した
