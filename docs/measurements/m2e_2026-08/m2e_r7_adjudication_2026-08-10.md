...
ゲートとして配線済み。**本手続きは実キャンペーンでは 08-10 に消化済み**
（現況 604/1280 = main 運転中。移入済み state では点検フラグも移入される）。
なお §6 ループのチャンクは時間区切りのため、仮に新規キャンペーンで使う場合、
点検ゲートの発火は「最初のチャンク境界かつ ≥174 セル」となり厳密に 174 で
止まらないことがある——ハーネスにセル数 cap を追加する厳密化は検証レイヤ変更
（キャンペーン中は generator code hash 凍結により禁止・E-系列の code change
厳禁と同旨）のため行わない。点検の意図（main 一括運転前の store_B 整合点検）は
チャンク境界発火でも保存される（基準①の件数一致は「その時点の期待件数」に
対して照合する）。
