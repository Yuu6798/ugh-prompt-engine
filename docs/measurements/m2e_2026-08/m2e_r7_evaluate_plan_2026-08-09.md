# M2e r7 (evaluate → store_B) 実行計画 — 承認済み・実行中（旧題: 実行計画案 — 設計側承認待ち）

- **作成日**: 2026-08-09 (JST)
- **状態**: ~~提案のみ・未開始~~ → **承認済み・実行中**（下記 08-10/08-11 追記が正。
  「提案のみ・未開始」は起草時点の記録）
...
- **追記（2026-08-11）**: §4 の単価監視（>500 s/cell×2 連続）は 08-10 の HALT 裁定
  （604/1280 セル時点・arm 構成シフト+resume 走査固定費の混入と診断）で**廃止・
  arm 正規化 3×median 規則へ差し替え済み**。正規仕様 = 裁定 memo §8.1。
  その他の予算値・run 上限は変更なし。
...
- **run 回数上限（提案）**: 水準あたり 15 + 3 = **18 run**、全体 **72 run** を上限とし、
  ...
  **【2026-08-10 確定】** 設計側裁定により 18/72 で確定（裁定 memo §5）。
...
- 単位コスト監視: チャンク毎に s/cell を記録し、**採用単価 335 s/cell を +50% 超過
  ...
  **【2026-08-11 廃止】** この計器は 08-10 HALT 裁定で廃止——現行の停止規則は
  裁定 memo §8.1 の **arm 正規化 3×median 規則**が正（本項は起草時点の記録として保持）。
...
設計側の確認を得てから起動する。
**【2026-08-10 充足】** 本条件は充足済み——予算値は設計側承認済み（裁定 memo §5）で
あり、キャンペーンは実行中（冒頭の状態追記が正。本節は起草時点の記録として保持）。
