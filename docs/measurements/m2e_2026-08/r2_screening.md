...
**ただし digest 一致までは保証しない（宣言された穴・2026-08-01 追記）。** 凍結して
あるのは**生成条件**（窓長・hop・軸・dB レンジ・カラーマップ）であってラスタライズ
実装ではない。matplotlib / numpy / libpng の版が違えば、同じ入力・同じ定数でも PNG
bytes は変わりうる。**この 50 枚を描いた時点の描画スタックは記録していない**ので、
第三者が得るのは「同条件の絵」であって「同一 bytes」ではない。以後の実行では
`spectrogram.py` が `renderer`（python / backend / matplotlib / numpy / pillow /
soundfile の版）を digest map に同梱する——**遡って埋めることはしない**（実行時に
存在しなかった記録を後から書けば、それは記録ではなく推定である）。
