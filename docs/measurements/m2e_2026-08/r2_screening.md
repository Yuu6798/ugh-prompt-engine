...
- **index 05（BKS - Too Much）**: −40 dB 期には生存候補だったので予備として判定した
  （該当なし）。**閾値を −26.0 dB に確定した結果、05 は (d) 該当になった**
  （`N_drop = 106` = 1.2307 s）ため、現行定義では生存候補ではない。
- **index 16（Hollow Ground - Ill Fate）**: 上記の変化により**現行定義での次点**
  （3 番目の生存候補・`N_drop = 76`）となったので、予備を張り直すために追加判定した。
