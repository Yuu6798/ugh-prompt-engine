...
| `evidence` | list[str] | `untested` 以外は 1 件以上必須。`^PR #\d+$` に fullmatch する項目（例 `"PR #171"`）のみ参照として素通し、それ以外は全件リポジトリ相対パスとして実在 + repo 内封じ込め（backslash・絶対パス・`..`・symlink 脱出を拒否）を検証（`/` の有無による判定ではない） |
| `depends_on` | list[str] | 参照先 id の実在必須・DAG（循環拒否） |
| `reentry` | Optional[str] | `status=dead` のとき必須、それ以外は任意 |
| `note` | Optional[str] | 自由記述の補足。`status=partial` のときは必須（境界記録） |
...
| `partial` | 部分的に裏付けられている（**`note` に境界を記すことが必須**、models.py が fail-fast で強制） |
...
`reentry` を必ず埋め、`partial` へ落とす/留まるときは `note` を必ず埋める
（境界記録なしの `partial` は fail-fast で拒否される）。
...
`partial` は `note` 必須 / status 語彙 / YAML 重複キー拒否 / `evidence` パス実在
+ repo 内封じ込め（backslash・絶対パス・`..` 成分・symlink 脱出を拒否 — 参照
として素通しするのは `^PR #\d+$` に fullmatch する項目のみ、§2 参照）。違反は
すべて集約し 1 回の `ValueError` にまとめて報告する。
