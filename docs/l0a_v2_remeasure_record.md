...
著者が単変数変更として設計・自己申告しているが、pin 済み score の
literal な差分には音響不活性な随伴編集が含まれる——s1 round2 は chorus2
の role 文（認識語彙なし・tier 解決不変）、s2 の両遷移は `meta.version`
（0.1→0.2→0.3、演奏入力に関与しない）——ことが事後監査で記録されている
（台帳 series_runs の各 rationale が正）。
