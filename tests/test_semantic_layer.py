...
def test_grv_anchor_secondary_dynamic_uses_legacy_valley_not_v2() -> None:
    # _infer_grv_anchor's hardcoded `valley_depth > 0.2` threshold is also a
    # frozen pre-v2-scale calibration (same policy as the config conditions
    # above). A high v2-scale valley_depth (0.5) must NOT add "dynamic" to
    # grv_anchor.secondary when valley_depth_legacy (0.05) stays below 0.2.
    phys = _make_physical_genre(
        harmonic_ratio=0.71,
        spectral_centroid=1800.0,
        low_ratio=0.20,
    ...
    assert "dynamic" not in semantic.grv_anchor.secondary


def test_delta_e_profile_uses_legacy_valley_not_v2() -> None:
    # _infer_delta_e_profile's `vd = phys.valley_depth` threshold ladder
    # (0.3 / 0.15) is likewise frozen pre-v2 calibration. A high v2-scale
    # valley_depth (0.5) alone would classify as "dramatic_contrast"
    # (vd > 0.3); with valley_depth_legacy (0.05) below both thresholds and
    # active_rate (0.6) below the sustained_energy gate (0.8), the profile
    # must resolve to "flat" instead.
    phys = _make_physical_genre(
        harmonic_ratio=0.71,
        spectral_centroid=1800.0,
        low_ratio=0.20,
    ...
    assert semantic.delta_e_profile.transition_type == "flat"
