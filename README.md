...
- [Melodia Confidence Scale Diagnosis](docs/melodia_confidence_scale.md) — why the Melodia route returned `voiced_coverage 0.000` on every M1-real material: not an adapter bug and not a demucs-stem mismatch, but a clash between the extractor's confidence semantics and the frozen gate (Melodia's `pitchConfidence` measured at max 0.176–0.295 vs the frozen `voiced_confidence_floor` of 0.30; pyin's `voiced_prob` tops out at 1.0). Pitch itself is recovered correctly — the instrument was never connected. How to normalize is a pre-registration matter, left undecided (diagnosis only)
