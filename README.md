...
- [Controllability PoC Planning](docs/controllability_poc.md) — Control track PoC (K-series): parameters as control knobs not eval values, grip effect-size definition, K0 → K2 Suno transfer, K3 orthogonality matrix (DCI/MIG effect-size reformulation, real Suno mini-matrix, device-specific sign flip)
...
- [control_profile](docs/control_profile.md) — PR1: `CompositionScore.control_profile` schema (per-generator grip_class self-description), sparse-allowed validation (vs fixity's full coverage), field→backend channel mapping, K2-derived Suno initial data
...
- [MusicGen Backend](docs/musicgen_backend.md) — Local MusicGen generation track: PR A (manual runbook + `musicgen` extra, no real inference, CI-safe) / PR B (real batch → K2-shaped fixture + `device_profiles/musicgen.yaml` + first R3 measurement) / PR C (R3 harness); DD-A deterministic contract (fixture→grip only in CI), generator-side so annotation isolation policy does not apply, weights license verified CC-BY-NC-4.0 (research-instrument only, weights never bundled)
