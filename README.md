...
- [M2d Error Model](docs/m2_error_model.md) — the crepe-route error model, the first input to M3: derived from the committed M2b (S band) and M2c (V band) measurements, four axes (pitch-center error, octave-error rate, voicing false-alarm as the systematic weak axis, per-material variance), a calibrated-band map (`crepe_direct` calibrated; `demucs_vocals_then_crepe` uncalibrated pending V_fullstack), interpretation only — no new measurement
