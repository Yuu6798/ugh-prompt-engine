...
- [MusicGen Backend](docs/musicgen_backend.md) — Local MusicGen generation track: PR A (manual runbook + `musicgen` extra, no real inference, CI-safe) / PR B (real batch → K2-shaped fixture, gated on G4 license verification) / PR C (R3 harness); DD-A deterministic contract (fixture→grip only in CI), generator-side so annotation isolation policy does not apply, weights license VERIFY PENDING
