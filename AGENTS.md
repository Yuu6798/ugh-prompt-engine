...
1. **実害基準で選別する**。採用の中心は「成果物の偽成功 / 偽失敗・データ破壊・
   provenance 汚染に至る具体経路が示される指摘」。**正典記録（台帳・記録 doc）の
   自己矛盾 — 撤回済み主張の残存・サマリーと per-round の不一致等 — も正直会計の
   毀損としてこの基準を満たす**（2026-08-06 PR #247 実測で明文化）。動作に実害の
   ない仕上げや逓減領域の深掘りは見送りが既定
...
3. **見送りは境界宣言で終端する**。保証範囲を明文で宣言して設計ノートへ記録し、
   ...
   **見送りスレッドは resolve しない** — 境界宣言の返信を付けたまま未解決で
   残置し、レビュアー / User が宣言を確認して最終判断できるようにする
   （黙って resolve すると見送りの存在自体が不可視になる。2026-08-06
   PR #247 実測で明文化）。**逆に採用して対応済みのスレッドは resolve する**
   （2026-08-20 追記・CLAUDE.md §bot レビュー対応の運用）。
   ...
   **複数メンバーを 1 つの理由でまとめて射程外にしない** — 共通理由（「この層を
   直すには X の構造変更が要る」等）を実地確認せずに全メンバーへ適用すると、
   実は既存 API で閉じられるメンバーが宣言に紛れ込む（2026-08-20 実測: PR #289
   §9 member 5 は「gate_synth の I/O 構造変更が必要」を 3 メンバーへ一括適用したが、
   楽譜モジュールは `load_song_module` が消費 digest を返しており不要だった）。
   宣言は**メンバーごとに理由を確かめてから**引く
4. **3 巡を超えて同一領域の指摘が続いたら逐次対応をやめる**。境界宣言で打ち切るか、
   マージ判断を User へ提案して収束させる（**1 PR あたりの通算上限は 10 ラウンド** —
   2026-08-20 追記・CLAUDE.md §bot レビュー対応の運用）（レビュー巡数は品質指標であると同時に
   ...
   **通算 10 巡は機械的に打ち切る**（2026-09-06 v1.2 運用固定）: 第 10 巡で
   未対応分を境界宣言（再開条件つき）にまとめレビュアーから unsubscribe し、
   残りは次周 Design Memo の材料として STATUS.md に登録する。実害 3 分類の
   新しい具体経路を示す指摘でも、通算 10 巡を超えたら当該 PR では採用せず
   次周 PR へ回す（打ち切りは 3 分類の適用可否を上書きしない——採用の判断は
   変わらないが、実行するタイミングだけ次周へ送る）。PR #346 の 26 巡
   （通算上限の明文化前で、実害基準に基づく正当な採用の連鎖だった）を
   反例として記録する
5. **防御を足す修正は、同じコミットで「正規フローがまだ通ること」をテストで固定する**
   （2026-08-21 PR #299 実測で新設）。偽成功経路を塞ぐ変更は、塞ぐ側の分岐だけを
   テストして通してしまいやすく、**防御は動くが正規フローが死ぬ**型の回帰を生む。
   実測 2 件（いずれも自分の直前の修正が原因で、次巡のレビューで発覚）:

   - 「開封済み marker が消えていたら stale」ガードを入れたが、その marker
     （`key_reveal.json`）は正規フローの前段が毎回削除する追跡ファイルだった。
     結果、**新しい pack に対する正規の初回採点が必ず BLOCKED** になっていた
   - 検証側の digest 計算を変えたが、**生成側が旧式のまま残り**（整形で 1 行化されて
     いて置換が当たっていなかった）、両者が別 payload を hash する状態になった。
     **新規 pack の初回採点が必ず失敗する**

   したがって:

   - **正規フローの成功経路を踏むテストを必ず 1 本足す**。「防御が発動する」テスト
     だけでは、正規フローを壊した回帰を検出できない
   - **producer / verifier のように対になる実装を触るときは、片側検証ではなく往復を
     踏むテストを書く**（生成側が入れた値を検証側が受理することを実際に確認する）。
     片側だけのテストは対の不一致をすり抜けさせる（上記 2 件目は、片側検証のテストが
     あったにもかかわらず 2 度すり抜けた）
   - 同一 PR 内で **自分の修正由来の指摘が続けて出たら発散のサイン**。4 の巡数上限と
     独立に、その時点で打ち切って残りを境界宣言へ回す
6. **指摘ごとに段階（生成/測定/検証）と目的を付けて判定する**。段階 1・2（仮説どおりの
   生成物を出す・実測して仮説を検定する）の問いが未回答のうちは、段階 3（測定器・
   生成物の検証・堅牢化）の指摘は境界宣言で次周へ送る。実害 3 分類に該当しても段階が
   違えば当該 PR では採用しない（2026-09-06 RUN10-CAL の反省: v1.0–v1.1 の Codex
   指摘の大半は段階 3 で、段階 2 の答えが無いまま採用していた）
...
- `AGENTS.md` §1–§5
- `AGENTS.md` §8 — **見出し（`###` 一覧 = 規律カタログ）の走査は常時必須**。
  当該タスクが触れる領域（例: スキーマガードを書く → truthy 判定則、記録の
  訂正 → 掃討則、正典データ編集 → 形状 enforce 則、docs 例文 → 例文仕様則、
  情報遮断実験 → ペイロード機械化則、spawn 配送 → 逐語性機械照合則）に
  該当する規律は本文まで読む
  （§8 は再発防止則の集積であり、該当規律を未読のまま作業すると防止対象の
  defect family が再発する）
...
11. **分岐軸を増やしたら fixture も増やす** — 新しい導出軸（判定/分類の場合分けを
    増やすフィールド）は、その軸が None でない行を含む fixture テストとセットでのみ
    入れる。導出ロジックの複製は禁止する（同じ判定を 2 箇所に別々に書かない — 1 箇所を
    正本にし、他は正本を呼ぶ）。出典: RUN10-CAL v1.1 §V3.5（D105）— `nuisance_axis`
    を含む `RowInput` 構築が `render_stage.py` にローカル複製され、`splitter.
    row_inputs_for_split()` との乖離が C4 入場不能（`BLOCKED_LEAKAGE`）を招いた。
    2026-09-06 v1.2 運用固定。
12. **永続成果物タスクは安全ゲートを組み込む** — 新規 loader / 配布物 / 永続成果物の
...
### 例文も仕様（ドキュメント例文の検証対象化。2026-08-06 制定）

契約・スキーマガイド・docs に載せる例文・サンプル値・テンプレートは、読者
（人間・LLM 著者の別を問わず）にとって本文と同格の仕様として機能する。
したがって例文は仕様本文と同じ検証対象とする: 実装と乖離した例文は仕様バグ
として扱い、乖離を見つけたら本文修正と同格で是正する。機械検証可能な例文
（スキーマ準拠の YAML/JSON 等）は、可能ならテストで実際に検証器へ通して
乖離を自動検出する。出典: 2026-08-05 L0-s スパイク（PR #245）実測 —
情報遮断された著者は契約の例文を仕様として直接消費し、例文の形式が
著者の出力形式をそのまま規定した（例文の誤りは著者の誤りに直結する）。

### truthy 判定を正規形ガードに使わない（is not None 厳密比較。2026-08-06 制定）

正規形スキーマのガードは、意図する制約に合わせて比較を選ぶ。**不在チェック
（「フィールドが与えられたか」）を truthy 判定（`if value:` / `if not value:`）で
書いてはならず**、`is not None` 等の厳密比較で書く — truthy 判定は空リスト・
空文字列・0 を「不在」と同一視し、「存在するが空」という正規形上有意な状態を
素通しする穴になる。逆に**非空制約（空も不正として拒否したい）には truthy
判定が正しく**、`is None` へ機械的に置換してはならない（置換すると
「存在するが空」を受理する逆方向の穴になる。例: fail 状態で `errors: []` を
拒否するガード）。禁止対象は「意図が不在チェックなのに truthy で書く」こと
であり、書く前に「空値はこのガードで有効か不正か」を判別してから比較を選ぶ。出典:
2026-08-05 L0a（PR #246）実測 — 同一モジュールで同型穴が 2 回発生
（`status='pass'` + `errors: []` の矛盾許容、`observed_sections: []` の
成功側素通し）。2 回目の発生時に本則へ一般化。

### 情報遮断実験のペイロード組成は機械化する（coordinator 散文注記の禁止。2026-08-06 制定）

可視ペイロードを宣言列挙する情報遮断実験（L 系列 D2 等）では、被験者へ渡す
ペイロードの組成を機械的（宣言済みファイルの転写のみ）に保ち、コーディネーターの
散文注記・要約・示唆を一切加えない。「ペイロード内から導出可能な事実の要約」で
あっても、散文注記は追加入力として非決定論的な被験者を操舵しうるため
off-contract イベントに当たる。注記が混入した周回の成果は収束実績・成功証拠に
算入できない。出典: 2026-08-06 L0b（PR #247）実測 — 注記の有無で著者の
レバー選択が実際に分岐（注記あり = 和声、注記なし = sub bass 統一）し、
唯一の改善実測が off-contract 除外され、最後の未汚染周回から系列を丸ごと
再実行するコスト（クリーン系列 3 周回）を要した。

### 情報遮断被験者への spawn 配送は逐語性を機械照合する（手動転写の禁止。2026-08-07 制定）

情報遮断実験の被験者 spawn では、組成済み author プロンプトの手動転写
（人手・モデル手のいずれも）を配送経路にしない。配送直後に、実際に配送
されたプロンプト本文（task JSONL 等の配送記録）と組成済み
`author_prompt.txt` を機械照合（strip 後バイト同一）し、一致を確認して
から応答を消費する。不一致を検出したら応答生成前に kill し spawn 前中断
として記録する（応答が汚染入力を消費していなければ off-contract には
数えない）。出典: 2026-08-07 PR #250 実測 — 手動転写で 1 文字破損が
2 件発生（「が」→「April」・「同」→「異」）、いずれも spawn 直後照合で
検出し実害ゼロで止血。照合を怠れば破損ペイロードの応答を正規周回として
消費し、系列再実行コスト（#247 のクリーン系列再周回と同型）を負う。

### 撤回・訂正の同語横断全数掃討（stale 主張の grep 掃討。2026-08-06 制定）

結論・帰属・呼称を撤回または訂正するときは、当該主張を表す語（例:
「操舵証拠」「前提条件」「クリーン再周回」）で**リポジトリ全域（docs・
データファイル・ソース・テストを含む）を横断 grep し、残存箇所を同一
コミットで全数掃討**する。テスト期待値に残った旧主張は、後続実装が旧状態を
復活させるか回帰テストを赤くする形で再発する。要約・per-round 記録・
見出し・ポインタ・テスト期待値は同じ主張を複製しており、1 箇所の訂正では
消費者がどこかで撤回前の結論を受け取る。§3 の「同型穴のファミリー掃討」の
散文版。**掃討対象は現行の正典主張（サマリー・見出し・per-round 解釈・
テスト期待値・ポインタ等、読者が現在の結論として消費する箇所）に限る** —
`correction_history` や履歴的 evidence（推定→確認→訂正の透明記録）は監査
証跡であり削除・改変しない。旧主張が履歴として現れる箇所は「訂正済み」で
あることが文脈から判別できる形（correction_history 内・訂正注記付き）で
保持する（本節の「実験 provenance の推定補完禁止」が要求する全段階の透明
記録と整合させる）。出典: 2026-08-06 PR #247 実測 — 撤回済み主張の残存が計 5 巡分の
レビュー往復（見出し・台帳サマリー・per-round 解釈・status キー・D2 会計）を
生んだ。訂正時の横断 grep を最初から行っていれば 1 巡で閉じた。

### parse 可能 ≠ 形状正しい（正典データ構造の形状 enforce テスト。2026-08-06 制定）

台帳・manifest 等の正典データファイルは「YAML/JSON として parse できる」
ことと「意図した形状（ネスト位置・キー集合・列挙順）である」ことが独立に
壊れる。手編集で維持する正典ファイルには、消費者が依存する形状（セクションの
親子関係・必須キー・エントリ列）を assert する形状 enforce テストを付ける。
編集時の safe_load 確認だけでは挿入位置ミスによるネスト退行を検出できない。
出典: 2026-08-06 PR #247 実測 — ledger への周回エントリ挿入位置ミスで、
クリーン再周回の記録が別セクション配下へ落ち（parse は成功）、消費者から
不可視になる退行が実発生。形状テスト追加後は同型退行を 2 回自動検出した。
...
追記（2026-08-10・同日実測による補強）: committed 実測記録を正とする前に
**鮮度の照合**を行うこと——当該記録より後の関連 commit（実装コード・マージ
履歴、`git log` での後続変更）と突合し、記録が supersede されていないかを
確認する。supersession の dated 追記を持つ記録はその追記側を正とする。
実例: r7 blocker 記録の「裁定待ち・停止中」は PR #254（08-09）の是正マージで
supersede されていたが記録未更新のまま残り、翌セッションが一次ソース照合を
行ったにもかかわらず stale な handoff を再生産した
（`docs/measurements/m2e_2026-08/m2e_r7_adjudication_2026-08-10.md` §0）。
committed であることは鮮度を保証しない——「一次ソース」の最上位は
「一次ソース + その後続 git 履歴」の組である。

### 回収・検収系の成功条件は閉世界契約で書く（要求集合の実行時導出禁止。2026-08-23 制定）

成果物の回収・検収で「全部揃ったら成功」を判定するとき、**要求集合
（何が揃っていなければならないか）を実行時データから導出してはならない**。
transport（directory listing・配信一覧）由来の要求集合は publish 失敗した
ファイルが集合ごと消えて検査を素通りし、record（結果 JSON の記載）由来の
要求集合も schema 差・digest 欠落で空集合に縮退して「何も検査せず成功」の
偽成功経路になる。測定契約が固定なら要求集合は**コンパイル時定数
（閉世界契約）**として書き、record は全エントリに well-formed な値を供給
しなければ失敗、全エントリの今回取得 + 照合成功のみを成功とする。
出典: PR #312（2026-08-23）実測 — fetch の成功判定が「マーカー存在 →
listing 由来 → record 由来」と 3 段階の実行時導出を経て、Codex 第 2〜5 巡が
各段の空洞化経路（stale マーカー受理 / listed-but-failed 黙殺 / 未 publish
素通り / 空 record 素通り）を同型連鎖で指摘。修正のたびに変形が再生産され、
第 5 巡の固定契約定数化（3 話者 × 2 曲 + onnx の 7 エントリ）で初めて
構造終端した。有償 pod の回収では偽成功 exit 0 → terminate で検証可能な
証拠が恒久喪失するため、この family の実害は特に大きい。
