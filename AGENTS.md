...
2. **レビュー対応の採否はオーバーエンジニアリング回避を第一規律とする**:
   ...
   境界宣言はスレッド返信と docstring の両方に残し、以降の同型指摘の
   打ち切り根拠にする。採否判定は Fable（設計判断）、スレッド取得・返信・
   修正実装は Sonnet/Opus 委譲
3. **対応済みスレッドは resolve 表示まで行って完了**: 修正 push → 対応内容の
   スレッド返信（採用/見送りと commit hash、attribution footer 付き）→
   `resolve_review_thread` で resolved 化、の 3 点セットが 1 指摘の完了条件。
   返信前にスレッド本文を照合してから投稿する（誤スレッド投稿の再発防止）
