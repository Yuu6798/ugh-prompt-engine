...
正規形スキーマの「フィールドの有無・空値の扱い」を強制するガードでは、
Python の truthy 判定（`if value:` / `if not value:`）を使わず、`is not None`
等の厳密比較で書く。truthy 判定は空リスト・空文字列・0 を「不在」と同一視し、
「存在するが空」という正規形上有意な状態を素通しする穴になる。出典:
