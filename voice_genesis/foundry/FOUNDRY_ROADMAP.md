...
| **S1 P2-PoC** | community ベース ckpt + **PJS 27 分** fine-tune → さくら/うみ | データ = PJS 全量 27 分（voicebank fine-tune の community 実務レンジ 15 分〜2h の下限側・これ以上減らす理由なし）。5〜10 万 step 級 | **$5–15**（4090 スポット 8–24h）・データ準備と runbook は本環境、GPU 実行は User | 自前パイプライン一周の音質は土俵に乗るか |
