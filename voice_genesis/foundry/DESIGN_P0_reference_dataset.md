...
- 第 1 バッチ: **系統×2,500 = 1 万本**（3 秒平均 @24k 16bit ≈ 1.5GB + 特徴量）。
  本環境の残ディスク（~26GB）に対し安全
- スキーマと manifest は 10 万本級へそのままスケールする設計（shard 前提）
- 生成スループットを record に実測記録（10 万本級の所要見積りに使う）
