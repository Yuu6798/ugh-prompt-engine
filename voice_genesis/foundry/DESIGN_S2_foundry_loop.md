...
- 状態: 設計 → バッチ1実装（本ドキュメントと同一セッションで着手）
...
- [ ] バッチ1の4体（lerp α=0.2/0.5/0.8 + slerp α=0.5）が合成され、
      `candidate_A.wav`〜`candidate_D.wav`（ブラインドシャッフル済み）として
      判定に付されること
- [ ] `results_s2/genome_ledger.json` に4候補が記録されること（§7 スキーマ）
- [ ] `results_s2/s2_record_2026-08-16.md` に生成条件・WAV sha256・対応表
      sha256・決定論確認・RMS/非無音確認が記載されること（判定4問は枠のみ）
- [ ] （後続）α×β の判定確定（User 耳判定後に本ドキュメントまたは record を
      更新）
