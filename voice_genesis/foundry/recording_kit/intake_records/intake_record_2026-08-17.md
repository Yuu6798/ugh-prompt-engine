...
**採用判定ではない**。以下は正式採用（Phase C `convert_user.py` / run 4 投入）前に
User 確認を要する:

1. **UC-001/UC-002 の曲同定**（確信度=中）: さくら/うみの割当は音響構造からの
   推定。User の一言確認または score.py/score_umi.py との DTW 照合で確定させる
2. **UC-003「あ」/ UC-007「お」の母音対応**（確信度=medium）: フォルマント
   推定は 1 対 1 で矛盾なしだが境界帯。耳確認で確定させる
3. **UC-017 のクリッピング実害**: 167 サンプル (0.042%)・代替テイクなし。
   可聴歪みの耳確認、必要なら再録（任意・非ブロッカー）
