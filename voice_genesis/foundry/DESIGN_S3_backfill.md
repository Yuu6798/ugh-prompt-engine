...
- 1 パス（sakura 25.4s + umi 27.0s + d3_sustain 63.5s + d3_kana 20.5s ≈ 136.4s）
  × seed 10 個 = 40 セル ≈ **22.7 分** → 目標帯に入る。
...
`.claude/memory/2026-08-17_design_next_session.md` §2-C のとおり。
歌詞既知（カード指定文句）→ 音素列確定、時刻は強制アラインメント。
受け入れ条件は「単独で強いアンカー」ではなく「spk_embed が立ち、補間三角形の
第 3 頂点として方向が聞き分けられる」。**着手は User 確認（UC-003/007・UC-001/002）後**。
...
- [ ] D3 データセットが build_dataset ゲートを通過し実効分数が record に記録される
...
- **Q3**: UC-017 再録の要否（任意）と T3（UC-018〜020）の追加収録
