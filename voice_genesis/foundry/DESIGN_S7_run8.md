...
#### 8-5-2b. **target 秒数の上限を先に置く**（★ User 裁定が要る・2026-08-20）
...
1 イベント（3 モーラ・助走 2 音 + 終端音）の voiced 実尺（固定テンポ 72 BPM・
1 拍 = 0.8333 s）:

| 終端の尺 | 1 イベントの voiced |
|---|---|
| 1 拍 | 0.833×2 + 0.833 = **2.50 s** |
| 2 拍 | 0.833×2 + 1.667 = **3.33 s** |
| 4 拍 | 0.833×2 + 3.333 = **5.00 s** |

§8-5-3 の要求（**セル間でイベント数を揃える**）より、1 セルあたり `E` イベント
とすると 9 セル合計は `E × 3 × (2.50 + 3.33 + 5.00) = E × 32.5 s`:
...
**事前登録する上限**: `target_voiced <= 0.60 × train_user_dose.voiced`
（= 103.1 s）。この上限から **E = 3** が一意に決まる。

**★ これは User 裁定 §8-1 の「4 語 × 2 回 = 72 イベント」を変更する。**
黙って変えないので、次のいずれかを User が選ぶ:

| 選択肢 | 帰結 |
|---|---|
| **(i) E = 3 を採る**（本書の既定） | イベント数 72 → 27。dosage 固定と 9 セル均等の両方を保てる |
| (ii) E を増やして target 比を上げる | 60% を超えると retained baseline が痩せ、(a) 完全置換へ漸近する（§8-5-1 の退化） |
| (iii) dosage 固定を緩める | §8-3 の単一介入会計が崩れ、結論が「target 内容 **または** 量増加」に限定される（§8-5-6） |
...
```
収録前ゲート（Run 8-0 の最後に実行・fail-closed）:
  1. 既存 15 row の (voiced, total ph_dur) を実測して台帳化する
  2. target 側の SP budget を明示し、target の total ph_dur を算出する
  3. §8-5-4 の決定論選抜を**実データに対して空回し**し、
     voiced と total の両方が ±1% に入る 6 row 部分集合が存在するか検査する
  4. 存在しなければ E や target 比を本 memo の改訂で決め直す
     -> **収録を発注しない**
```
...
= **27 イベント**（§8-5-2b の上限から導出）。**セル間で秒数が違うのは
設計どおり**である（1 拍セル 7.5 s / 4 拍セル 15.0 s）。
