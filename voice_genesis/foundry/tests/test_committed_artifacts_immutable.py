...
1. sha256 が `debt_ledger.yaml` の対応 debt の `evidence_delivered` に記帳
   された値と一致する（**台帳から動的に読む**。sha256 をこのテスト内へ
   ハードコードしない — 台帳側が改訂されずファイルだけ変わる事故と、
   ファイルは無事で台帳側だけ書き換わる事故の両方を検出するため）。
