...
- [x] `requirements_run5_pod.lock` の PENDING 解消（gates/binarize 2 段の
  実測 freeze から。**venv は無く同一インタプリタが binarize 段で in-place
  降格される**ことが判明したため、決定論の担保が STAGE_PLAN の順序不変条件に
  依存する点も明記）
- [x] 費用 ≤ $4（**実測 ≈$1.40**）
...
- Q10 / Q11: §1.2 の通り run 6 の主検証点 → **裁定済み**（record §6.2/§6.3。
  Q10 = 達成〔②無留保の「第三の声」確定〕— ただし帰属は「正規化を含む run 6
  の条件下で到達」までで、別走行ゆえ正規化単独の効果とは分離できない /
  Q11 = 音量の区間揺れは残存し、発生源はカード内ダイナミクスまたは
  モデル/曲側 = 入口正規化の射程外と確定。④ user 寄りセル不安定の再評価は
  VG-E1 へ送り・T3 は任意のまま）
