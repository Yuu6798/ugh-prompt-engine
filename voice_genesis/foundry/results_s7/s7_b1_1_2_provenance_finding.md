...
なお **「対応を記録する」だけでは足りない**（PR #303 第 3 巡で指摘）。2 つのパスを
別々に hash して並べた記録は、観測していない関係を証明したことにする。記録は
**export を実行したプロセス自身が出す**必要があり、`s7_export_manifest.py` は
束縛の由来を `binding_evidence` として区別する（`witnessed_export` =
空の出力先に対して exporter を起動し現れた物を hash した / `unwitnessed_post_hoc` =
既に在る物を後から hash しただけ）。レンダ側は既定で前者しか受け付けない。
