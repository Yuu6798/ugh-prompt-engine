...
## 7. 耳判定記録（2026-08-15 追記・判定者 = User・非盲検）

判定者所見（原文）:
- SawSing 2 本: 「人工音声としては破綻が少ない。ただ音程は破綻に近いくらいバラバラで不安定」
- GOLF 系: 「音程はまあまあ取れているがノイズがあったり子音が不安定など音声として難あり」
- 総評: 「どちらが良いと言い切るほど材料が無い」→ **経路裁定は保留**

機序判読（記録者）:
- SawSing の音程不安定は、mel から f0 を**ネットワークが予測する**推論経路 + OOD 入力
  （v5 合成暗声は MPop600 学習分布の外）の複合と推定。工房の使用形態では f0 は
  score / Performance 遺伝子から**明示駆動**するため、この欠点は工房経路に転移しない
  見込み（要実証）。テクスチャ所見（破綻少）は DSP 合成層の性質でありそのまま転移する
- GOLF の負所見は speech checkpoint（VCTK 話し声学習）への歌唱 OOD 入力という交絡を
  含む。歌声 checkpoint（ISMIR23 / MPop600）は未実行
- 帰結: 両者の負所見とも F0 スモークの試験条件に固有の成分を含み、裁定材料として
  不完全。次段 = 明示 f0 駆動での A/B（F1a）で交絡を除去してから裁定する
