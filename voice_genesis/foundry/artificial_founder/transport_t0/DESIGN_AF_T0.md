...
コミット済みコードから再計算した閉包ダイジェストが一致し、構成ファイルにも
drift が無い。

`SHA256SUMS.txt` が名指しする **全エントリを版管理へ出荷する**。マニフェストと
出荷範囲が食い違うと、クリーンチェックアウトで `sha256sum -c` が失敗し、
「全エントリ一致」という記録の主張が偽になる（レビュー第 7 巡の指摘）。
当初は AF-P0 の前例に倣って輸送後 WAV / sidecar / 段台帳を除外していたが、
実測で JSON 記録 約 412K + WAV 約 552K = 約 1MB と小さく、**自己検証可能な
