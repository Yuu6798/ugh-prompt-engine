"""VT-3 v5: 軸別測定帯域による grip 完成（design memo v05 §A, §B）。

v4 の教訓: joint fit の失敗根因はアルゴリズムではなく **probe suite の
中高音域における倍音サンプリング不足**（f0 がフォルマント分離幅 350Hz と
同オーダー以上になると 2 ピーク分解が情報量的に不可能）。
本サイクルはアルゴリズム（measure_v4.py の joint fit、v4 の安全機構込み）
を一切変更せず、formant_scale / spectral_tilt の 2 軸にのみ **低音域
measurement band** {G2,C3,E3,G3,C4}（f0 98-262Hz）を割り当てる。
breathiness / vibrato_depth は v4 で既に PASS 済みのため再測せず結果を
re-state する（design memo v05 §A）。
"""
...
from vt3_v3 import AXES, MID_SWEEP_INDEX, PROBE_MIDIS as PROBE_MIDIS_STANDARD, REPEAT_SEED_OFFSETS, _with_seed_offset
from vt3_v4 import AXIS_MECHANISM_NOTES as V4_MECHANISM_NOTES, extract_config

WORK = Path(__file__).parent
RESULTS = WORK / "results_v5"
RESULTS.mkdir(exist_ok=True)
RESULTS_V4 = WORK / "results_v4"

FEATURE_NAMES = m4.FEATURE_NAMES_V4
REF_SCALE = m4.REF_SCALE_V4
FMIN, FMAX = 55.0, 2900.0

# --- band 宣言（凍結、design memo v05 §A） ---
PROBE_MIDIS_LOW = {"G2": 43, "C3": 48, "E3": 52, "G3": 55, "C4": 60}  # f0 = 98.0-261.6Hz

BAND_ASSIGNMENT = {
    "breathiness": {"band": "standard", "probes": PROBE_MIDIS_STANDARD, "note": "v4でPASS済み。再測不要のためv4結果をre-state。"},
    "vibrato_depth": {"band": "standard", "probes": PROBE_MIDIS_STANDARD, "note": "v4でPASS済み。再測不要のためv4結果をre-state。"},
    "formant_scale": {
        "band": "low_register",
        "probes": PROBE_MIDIS_LOW,
        "note": (
            "中高音域(C3超)はf0がF1-F2分離幅(350Hz)と同オーダー以上になり"
            "2ピーク分解が情報量的に不可能(v4で実測確認)。低音域{G2,C3,E3,G3,C4}"
            "(f0 98-262Hz)は4kHz以下に倍音15本以上を含み2ピーク分解が情報量的に"
            "成立する帯域として宣言する。中高音域tract特徴はout_of_bandとして"
            "明示的に除外する(設計書§1.5eと同型の原理: 計器分解能外を「測って"
            "失敗」ではなく「計器適用範囲外」として扱う)。"
        ),
    },
    "spectral_tilt": {
        "band": "low_register",
        "probes": PROBE_MIDIS_LOW,
        "note": (
            "formant_scaleと同一の根拠(倍音サンプリング不足による2ピーク分解"
            "不能)によりsource_tiltの品質もF1_est/joint fitの安定性に依存する"
            "ため、同じ低音域bandを割り当てる。中高音域はout_of_band。"
        ),
    },
}

OUT_OF_BAND_PROBES = {
    "formant_scale": {k: v for k, v in PROBE_MIDIS_STANDARD.items() if k not in PROBE_MIDIS_LOW},
    "spectral_tilt": {k: v for k, v in PROBE_MIDIS_STANDARD.items() if k not in PROBE_MIDIS_LOW},
}
...
def evaluate_axis(axis_name: str, axis_spec: dict, axis_data: dict) -> dict:
    ...
    n_2peak = sum(v for k, v in fit_dist.items() if k == "joint_2peak")
    n_1peak_fallback = n_cells - n_2peak
    ...
    result = {
        ...
        "n_1peak_fallback_cells": n_1peak_fallback,
    ...
    if not pass_no_exemption and dominant_side is not None:
        ...
        exemption = {
            ...
            "eligibility_evidence": (
                f"dominant_side after joint-fit remains '{declared}' with sign consistency "
...
def restate_from_v4(axis_name: str) -> dict:
    """v4 で PASS 済みの軸を再測せず results_v4/grip_report_v4.json から re-state する。"""
    ...
    restated["restated_from"] = "results_v4/grip_report_v4.json (v4で既にPASS。design memo v05 §A の指示により再測せず re-state)"
    restated["final_gate_pass"] = v4_axis["final_gate_pass"] if "final_gate_pass" in v4_axis else v4_axis["no_exemption"]["overall_gate_pass"]
...
def main() -> None:
    ...
    for axis_name in ["breathiness", "vibrato_depth"]:
        print(f"=== axis: {axis_name} (re-stated from v4, band={BAND_ASSIGNMENT[axis_name]['band']}) ===")
    ...
    for axis_name in ["formant_scale", "spectral_tilt"]:
        ...
        probe_midis = BAND_ASSIGNMENT[axis_name]["probes"]
        print(f"=== axis: {axis_name} (band={BAND_ASSIGNMENT[axis_name]['band']}, probes={list(probe_midis.keys())}) ===")
    ...
    with open(RESULTS / "grip_report_v5.json", "w") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)

    print(f"\nVT-3 v5 elapsed={report['elapsed_sec']}s")
