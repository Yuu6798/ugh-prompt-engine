...
def test_legacy_publish_is_disabled_and_leaves_no_bundle(tmp_path: Path) -> None:
    ...
    with pytest.raises(bp.BirthProbeError, match="success-only admission harness"):
        bp.publish_evidence_bundle(output, result, observations, pjs)
    assert not output.exists()
    assert not list(tmp_path.glob(".evidence.build-*"))
...
def test_gate_synth_renderer_rejects_non_exact_c1_profile_before_synthesis() -> None:
    ...
    class FakeGate:
        ...
        def run_pipeline(*args, **kwargs):
            raise AssertionError("gate_synth must not run with a malformed C1 ControlProfile")
    ...
    with pytest.raises(bp.BirthProbeError, match="C1 must carry the exact derived CONTROL profile"):
