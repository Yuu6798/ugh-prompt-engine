...
- ~~**消費バイト hash が 6 バッファ止まり**~~ — **この据え置きは全面撤回した**。
  楽譜モジュールは `load_song_module` の戻り値で（7 巡目）、embed / 音素辞書は
  `*_with_sha` ローダで（12 巡目）**いずれも gate_synth 無改変で閉じられた**。
  「I/O 構造変更が必要」は 2 度とも事実誤認だった。現状は
  `not_covered` が**空**で、member 4・5 は**実装済み・未実測**（再実測で昇格）
...
2. **§9 member 5 = 実装済み・未実測**（話者 embed / 音素辞書。`*_with_sha` で
   照合を実装済み。射程外宣言は 12 巡目に撤回）
...
### bot 13 巡目（1 件・**実害例外として採用**）— 撤回済み主張が現在形で残っていた

| 指摘 | 中身 | 判定 |
|---|---|---|
| Remove the withdrawn six-buffer scope-out | §9-2 の据え置き一覧が「embed / 音素辞書は gate_synth のパス read で、閉じるには I/O 構造変更が必要」と**現在形のまま**残り、12 巡目で訂正した member 表と同一記録内で矛盾していた | **採用**（正典記録の自己矛盾 = 正直会計の毀損。CLAUDE.md の実害基準に該当） |

**「1 箇所直して兄弟を残す」が 6 回目**（STATUS / 記録本体 / `conclusion` /
`purpose` / member 4 の 4 箇所 / 今回）。今回は指摘の 1 行を直す前に、記録全体を
`I/O 構造変更|パス read|6 バッファ|射程外` で grep して**現在形の主張を全数
列挙**し、2 箇所（§9-2 の据え置き / 10 巡目の引き継ぎ一覧）を同時に直した。
残りのヒットは撤回の経緯記述で、履歴として保持すべきもの。

**§9-2 の当該項目は打ち消し線で残す**（削除しない）。据え置き判断が撤回された
こと自体が、「閉じられないと書く前に API を grep する」の実例として読める形に
しておくため。
