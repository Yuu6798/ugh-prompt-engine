...
| | `candidates/registry.py` | `Candidate.abstention_reasons: frozenset[vocab.MissingReason] = frozenset()`（`M2A-B0-AUTOCORR-PERIODICITY` のみ `{OUTPUT_MISSING}` を宣言） | `D113`。`candidate_space_sha` 変化——pin は `candidate_space` ledger event（`artifact_sha`）。凍結 manifest の `candidates` セクション（path+hash のみ）とは別軸 |
