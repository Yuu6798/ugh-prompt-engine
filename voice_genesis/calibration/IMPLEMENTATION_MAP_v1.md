...
| | `campaign/selection_stage.py` | `candidate_fail_filter_report()`（`negative_control_undeclared_missing`——PR #354 round 3 finding #1 採用、新設の独立 fail filter。`negative_control_declared_abstentions` は audit-only のまま存続） | `D113`。round 3 是正で WP-A block 3 deviation を解消——holdout `_negative_fired()` v1.4 経路 (B) と同一述語（`abstained()`）を `(row_id, probe_index)` instance 単位で適用し、fire/non-fire 判定を holdout と一致させた（旧記述「selection 側は round-20 型契約を持たないため fire/non-fire 判定は不変」は上書き） |
