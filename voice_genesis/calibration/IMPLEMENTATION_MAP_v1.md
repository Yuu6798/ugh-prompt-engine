...
| | `c0_validate.py` | `_check_gate_approval_ordering()`（一致する承認記録が `approvals/records/` にあり順序逆転を検出できれば `GateApprovalOrderingReport.violations` → `missing_required_keys` へ合流し `BLOCKED_C0_MANIFEST_INCOMPLETE` で blocking。`C0ValidationResult.gate_approval_ordering_notes` は記録が見つからない等の検査不能時のみの非ブロッキング情報） | 検査不能（承認記録が `approvals/records/` に無い等）の場合のみ notes 側へ |
