...
## [UNDERSPEC-F-11] proto1_demo.py: publish フェーズの複数ファイル非トランザクション性（PR#261 レビュー R6, 見送り境界宣言）

`_publish_outputs()`（PR#261 レビュー R1）は registry・WAV・e2e_run.json の
staging→正本 `os.replace` を 1 箇所にまとめ、run_2/監査/WAV レンダ/
e2e_record 組み立てが全て成功した後にのみ実行するよう再構成した。これにより
不整合の発生し得る窓は「音声パイプライン実行中の数分間」から「連続する
`os.replace` 呼び出し間のミリ秒オーダー」まで縮小した。第 3 ラウンドの
レビューはこの残余窓（`_publish_outputs()` 内の連続 `os.replace` そのもの
が複数ファイルをまたぐ真のトランザクションではない点）を再度指摘したが、
以下の理由で**見送り（実害基準、AGENTS.md §3）とし、追加の修正は行わない**：

  (a) 発生確率が極めて低い（ミリ秒オーダーの窓、かつ os.replace 自体は
      単一ファイルなら atomic な rename システムコール）
  (b) 対象は git 管理下の demo 成果物であり、壊れても再実行で復旧可能
      （決定論パイプラインのため、正本が破損した場合は `python
      proto1_demo.py` の再実行で機械的に復元できる）
  (c) 完全なトランザクション化（`results_final/` を丸ごと staging
      ディレクトリで組み立て、単一の `os.replace` でディレクトリごと
      swap する設計）は、それ自体が「swap 失敗時のロールバック」という
      新しい失敗モードを持ち込む上、`results_final/` には本デモ以外が
      書く既存の committed 記録（設計メモ・acceptance_report 等）も
      同居するため、安全に導入するには別途設計が要る

`_publish_outputs()` の障害注入テスト（`tests/test_proto1_demo.py`）は、
この残余リスクを機械的に検出できることを示すと同時に、上記の意図的受容の
記録でもある。

**再入条件**（この見送りを再検討すべきタイミング）: registry/e2e_run.json
が本デモ（単発の手動実行、demo 成果物）の外で運用される場合——例えば
複数プロセスから並行 append される、または CI で定期実行され失敗時の
自動リトライ・自動復旧が要求される場合——は、`results_final/` 相当の
出力ディレクトリを単一 `os.replace` で丸ごと差し替える完全トランザクション
設計への刷新を検討すること。
