...
├── recast/                    # RecastProject: 既存 sidecar (CompositionScore/IdentityManifest/ArrangementSpec/InputCapabilityProfile) への参照+実行方針のみのワークスペース定義 (recast-project/0.1。models/loader/plan/state/backend + backends/(manual/deterministic/musicgen)。PR1 = schema+loader+fixture、PR2 = `svprpe recast plan`/`status` CLI + 状態機械 (recast-state/0.1) + mode_overrides (★invocation_mode 軸、mode-overrides/0.1)、PR3 = BackendInvoker 抽象 + manual 注文書(6ファイル) + deterministic backend(in-process 演奏者) + `svprpe recast run` CLI)
├── batch/                     # バッチ処理
