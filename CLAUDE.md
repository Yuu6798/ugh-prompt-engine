...
- **長時間検証の同期実行固定文言**: pytest 等の長時間検証を委譲するプロンプトには
  「①フォアグラウンド実行 ②timeout は Bash ツールの timeout パラメータで明示（例:
  600000ms・シェル `timeout` ラッパ代用不可）③run_in_background / `&` / nohup 禁止」
  の 3 点を必ず含める（07-07/07-08 事故×3: バックグラウンド化は委譲エージェント
  停止時に検証ごと死亡し、禁止文言の明記だけでは防げない）
- **孤児化の回復手順**: 委譲した検証が環境再起動等で孤児化したら、SendMessage で
