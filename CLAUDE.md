...
### bot レビュー対応の運用（2026-08-20 新設 / 2026-08-21 採否基準を明文化）

- **採否は 3 分類のみ**（2026-08-21 User 裁定）: **実コード被害 / 将来汚染**（誤った
  規約・帰属・スキーマが下流の実装や記録を汚す）**/ 致命的バグ**（矛盾・実行不能・
  偽成功経路）。他は境界宣言で止め、**修正からの再修正は早期に打ち切る**。**指摘は段階（生成/測定/検証）と目的を判定してから採否を決め、段階 1・2 が未回答のうちは段階 3 の堅牢化指摘は境界宣言で次周へ送る**（実害 3 分類に該当しても段階が違えば当該 PR では不採用。2026-09-06 RUN10-CAL 反省 = `AGENTS.md` §3 item 6）
- **上限 10 ラウンド**（同一領域 3 巡 = `AGENTS.md` §3-4 に加算）。第 10 巡で未対応分を
  境界宣言（再開条件つき）にまとめ unsubscribe、残りは次周 Design Memo 材料として STATUS.md へ登録。**打ち切りは 3 分類を上書きしない**（新経路の指摘でも通算 10 巡超は次周 PR 送り）
- **採否と方針はコミットメッセージに書く**（採用は何を直したか / 見送りは理由）。返信
  不可の環境でも PR タイムラインに記録が残る
- **対応済みスレッドは resolve する**（見送りは resolve せず境界宣言を付けて残置
  = `AGENTS.md` §3-3）。repo スコープの可否はセッションごとに違うため**「不可」と
  決めつけず毎回試し**、実際に不可のときだけ User へ引き継ぐ
