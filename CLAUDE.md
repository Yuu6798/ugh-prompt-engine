...
| [`docs/melodia_confidence_scale.md`](docs/melodia_confidence_scale.md) | M1-real 診断（2026-07-25）: Melodia 経路が全素材で `voiced_coverage 0.000` を返した原因は、アダプタ設定バグでも demucs stem の噛み合わせでもなく、**抽出器の信頼度セマンティクスと凍結ゲートの衝突**（Melodia の `pitchConfidence` 実測 max 0.176–0.295 < 凍結 `voiced_confidence_floor` 0.30。pyin の `voiced_prob` は max 1.0）。音高自体は正しく取れており「観測できなかった」のではなく計器が未接続だった。正規化方式は事前登録マターとして未決（診断のみ・実装なし） |
