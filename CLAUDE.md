...
| [`docs/recast_workspace.md`](docs/recast_workspace.md) | recast トラック総括（PR0–PR6）: recast-project/0.1 スキーマ・状態機械・CLI フロー（init→plan→run→ingest→status）、invocation_mode 軸（cover/prompt_only 実測差）、Phase 0 ゲート帰結（melody=not_observed、chords+structure が hard の柱）、「約束するのは測定できるものだけ」（D-1 準拠）、golden path の回し方 |
