...
### bot レビュー対応の運用（2026-08-20 新設）

- **上限 10 ラウンド**。同一領域 3 巡での打ち切り（`AGENTS.md` §3-4）に加え、
  1 PR あたり bot 再レビューは通算 10 巡を上限とする。超えたら未対応分を
  境界宣言にまとめ、マージ判断を User へ渡して収束させる
- **採否と方針はコミットメッセージに書く**。採用は何をどう直したか、見送りは
  理由（設計範囲・保証境界）を簡潔に記す。コミットは PR タイムラインに載るため、
  レビュースレッドへ返信できない実行環境でも採否の記録が PR 上に残る
- **対応済みスレッドは resolve する**。ただし**見送りは resolve しない**
  （`AGENTS.md` §3-3 = 境界宣言を付けたまま残置し、見送りの存在を可視に保つ）
