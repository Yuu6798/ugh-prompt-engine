...
適用範囲は「開発サイクルと機能追加の原則」節に従う（下記 3 分類は生成→実測のサイクルにある生成物にのみ適用し、測定器・検証器側への機能追加の根拠にはしない。2026-09-06 の「段階（生成/測定/検証）で判定する」規則 = `AGENTS.md` §3 item 6 は同節の原則 2–4 へ一般化して吸収した）。
