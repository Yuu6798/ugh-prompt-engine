...
- 2026-05-03 (Session 3): Q4'-6 Learned Output Validation Harness を 1 セッションで設計 → Codex 実装 → PR レビュー × 3 完走。PR #33 (本体) / #34 (skipped 集計分離・L-1〜L-4) / #35 (二重デコード解消) を順次マージ。採用 metric は downbeat F-measure (70 ms) + note onset+pitch F (50 ms / ±50 cents)、deterministic 側を frame→note ビン化して note-domain で統一。昇格ゲート G1–G5 を `docs/learned_models_policy.md` に策定（synth +0.05 / mean error 退行なし / real-audio ≥20 曲 win-rate 70% / license / determinism）。panns_inference は対象外、Q4'-7 promotion は real-audio dataset 整備待ち
