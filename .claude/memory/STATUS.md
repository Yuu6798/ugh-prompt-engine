...
弁別）に絞った。負の結果は docs に外部化。**2026-06-18 に R2 closeout 完了**（docs PR、
branch `claude/daily-task-checkin-juh04s`、User マージ待ち）— 完成定義 §4 を確定し **bpm を
...
| R2 closeout | bpm を再現対象に含むか明示除外か | P1 | docs PR を branch `claude/daily-task-checkin-juh04s` に push 済（User マージ待ち）。結論: **bpm を確率的経路(R3)の信頼再現ノブから明示除外**（faster-side=post-hoc 部分緩和・÷2=extractor 分離不能、R0 三値診断と screener prior 回復診断には残置）。roadmap_goal2.md §4 / R2-3（roundtrip_preservation.md per-field bpm trust）/ coverage.md へ伝播。**マージ後に本行を sweep（既マージ表へ移送）**。roadmap_goal2.md R2 |
