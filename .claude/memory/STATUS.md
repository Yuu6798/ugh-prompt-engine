...
2026-07-03（Session 3）、**デモ 4 部作からカバー商流一周まで**（マージ 0・全成果は未マージ branch `claude/fable-demo-project-t6czdl` + scratchpad + Artifact 4 頁）。デモ 4 部作（決定論 6/7 preserved / MusicGen ガチャ=bright 効く・bpm 効かない / Suno A/B=取説予言的中 / 機種間採点=リズム骨格は運べ音高系は運べない・プロンプト衛生で bpm 保存 2/5→5/5）→ **CLAP を判定器から転写器へ**（意味層 3 行置換の A/B で bright_contrast 全テイク符号反転・原曲類似 0.460→0.516・物理非劣化）→ センサー裁定（contrast 対=正 / zero-shot 順位=誤読 / 決定論 stem RMS が裁く・提供 4 曲は全インストと確定）→ **歌詞アンカー機械化**（torchaudio HDEMUCS + faster-whisper で実歌詞 25 セグメント転写、フルミックス直は幻覚=ステム分離必須）→ **実 Suno カバー発注→検収全項目クリア**（key 3/3・bright 3/3・vocal 3/3・歌詞フレーズ再現・原曲類似 0.75-0.77 ≫ 機種間 0.43-0.59）。R2 アトラクタを真値付き再確定（原曲 BPM=173 vs 素朴 117.45=3:2、検収 3 曲も 117.45×2+86.13×1 → 高 prior 3/3 で 172.27 回復）。インフラ: `semantic-embed`/demucs は py3.11 ビルド不能 → 回避経路確立（詳細 2026-07-03.md S3 Handoff）。
